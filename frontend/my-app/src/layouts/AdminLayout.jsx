...
const AdminLayout = () => {
    ...
    const menuItems = useMemo(() => {
        ...
        if (userRoles.includes(ROLES.ADMIN)) {
            items.push({ icon: FiHome, label: "Tổng quan", path: "/admin" });
            items.push({ icon: FiUsers, label: "Quản lý người dùng", path: "/admin/users" });
            items.push({ icon: FiSettings, label: "Cấu hình hệ thống", path: "/admin/system-config" });
        }

        // Lecturer Items
        if (userRoles.includes(ROLES.LECTURER)) {
            items.push({ icon: FiBook, label: "Quản lý giáo trình", path: "/lecturer/textbooks" });
        }

        // Reviewer Items (HoD, AA, Rector)
        if (userRoles.includes(ROLES.HOD) || userRoles.includes(ROLES.ACADEMIC_AFFAIRS) || userRoles.includes(ROLES.RECTOR)) {
            items.push({ icon: FiCheckSquare, label: "Duyệt giáo trình", path: "/review/dashboard" });
        }

        // Fallback or Common items if needed
        // items.push({icon: FiUser, label: "Profile", path: "/profile"});
    ...
    return (
        <div className="flex h-screen bg-gray-50 font-sans">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed lg:static top-0 left-0 h-full bg-gradient-to-b from-blue-900 to-blue-800 text-white shadow-xl z-30 transition-all duration-300 ease-in-out ${isSidebarOpen ? "w-64 translate-x-0" : "w-0 -translate-x-full"
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="h-20 flex items-center justify-between px-6 border-b border-blue-700">
                        <div className="flex items-center">
                            ...
                            <h1 className="text-2xl font-bold text-white">SMD</h1>
                        </div>
                        <button
                            onClick={() => setIsSidebarOpen(false)}
                            ...
                            {menuItems.map((item, index) => (
                                <li key={index}>
                                    <button
                                        ...
                                        className={`flex items-center w-full px-4 py-3 rounded-xl transition-all duration-200 group ${activePath === item.path
                                            ? "bg-white text-blue-700 shadow-lg"
                                            : "text-blue-100 hover:bg-blue-700 hover:text-white"
                                            }`}
                                    >
                                        <item.icon
                                            className={`w-5 h-5 mr-3 ${activePath === item.path ? "text-blue-700" : "group-hover:text-white"
                                                }`}
                                        />
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="ml-4 flex items-center space-x-2 text-sm text-gray-500">
                            ...
                            <span className="text-blue-600 font-medium">
                                {menuItems.find(item => item.path === activePath)?.label || "Tổng quan"}
                            </span>
                        </div>
                    </div>
                        ...
                        <div className="flex flex-col md:flex-row justify-between items-center space-y-2 md:space-y-0">
                            ...
                            <div className="flex items-center space-x-6">
                                <p className="text-sm text-gray-600">
                                    © 2024 SMD System. All rights reserved.
                                </p>
                                <div className="flex items-center space-x-4">
                                    <a
                                        href="#"
